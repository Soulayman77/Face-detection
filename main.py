...
counter = 0
referenceImgk = cv2.imread("venv/images/referenceImgk.jpg")
referenceImgS = cv2.imread("venv/images/referenceImgS.jpg")
################################################################################################################################################################
...
def check_face(frame):
    ...
    who = " "
    try:
        #verify if we already checked name1 in the frame and found her ,
        # because there can only be one name1 in the picture and there's no point checking twice
        if not ("name1" in checked_faces) and DeepFace.verify(frame, referenceImgk.copy())['verified']:
            who = "name1"

            #Adding name1 to the checked faces list so we dont check for her again this iteration
            checked_faces.append(who)

        elif not ("name2" in checked_faces) and DeepFace.verify(frame, referenceImgS.copy())['verified']:
            who = "name2"
            checked_faces.append(who)
        else:
            who = "no match"
